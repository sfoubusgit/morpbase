...
const STORAGE_KEY = 'promptgen:pool_hub_store:v14';
const CURRENT_STORE_VERSION = 14;
...
const loadStore = (): PoolHubStore => {
  try {
    ...
    if (!raw) {
      const v9 = window.localStorage.getItem('promptgen:pool_hub_store:v9');
      if (v9) {
        const migrated = migrateV4(v9);
        saveStore(migrated);
        return migrated;
      }
